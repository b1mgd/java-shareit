...
class ItemServiceImplTest {
    ...
    void setUp() {
        ...
        // Глобальные заглушки для всех findAll/findAllBy.../searchItems
        when(itemRepository.findAllByOwnerId(anyLong())).thenReturn(Collections.emptyList());
        when(itemRepository.searchItems(anyString())).thenReturn(Collections.emptyList());
        when(bookingRepository.findAllByItemIdIn(anyList())).thenReturn(Collections.emptyList());
        when(commentRepository.findAllByItemId(anyLong())).thenReturn(Collections.emptyList());
        when(commentRepository.findAllByItemOwnerId(anyLong())).thenReturn(Collections.emptyList());
        when(requestRepository.findById(anyLong())).thenReturn(Optional.empty());
        when(userRepository.findById(anyLong())).thenReturn(Optional.empty());
        when(itemRepository.findById(anyLong())).thenReturn(Optional.empty());
        // Для ownerId=1L всегда возвращаем пользователя
    ...
    void addComment_shouldThrowNotValidException_whenNoBooking() {
        ...
        when(userRepository.findById(authorId)).thenReturn(Optional.of(author));
        when(itemRepository.findById(itemId)).thenReturn(Optional.of(item));
        when(bookingRepository.findFirstByItemIdAndBookerIdAndStatusAndEndBefore(anyLong(), anyLong(), any(), any())).thenReturn(Optional.empty());
        assertThrows(ArgumentsNotValidException.class, () -> itemService.addComment(request, itemId, authorId));
    }
    ...
    void getAllCommentsForOwner_shouldReturnEmptyList_whenNoComments() {
        long ownerId = 1L;
        when(commentRepository.findAllByItemOwnerId(ownerId)).thenReturn(Collections.emptyList());
    ...
    void searchItems_shouldReturnList_whenTextIsNotEmpty() {
        ...
        Item item = new Item();
        item.setId(1L);
        when(itemRepository.searchItems(text)).thenReturn(List.of(item));
    ...
    void createItem_shouldReturnItemDto_whenValidRequest() {
        ...
        when(requestRepository.findById(0L)).thenReturn(Optional.of(new Request()));
        Item item = new Item();
    ...
    void patchItem_shouldReturnItemDto_whenOwner() {
        ...
        PatchItemRequest request = mock(PatchItemRequest.class);
        when(itemRepository.findById(itemId)).thenReturn(Optional.of(item));
        when(request.hasName()).thenReturn(false);
        when(request.hasDescription()).thenReturn(false);
        when(request.hasAvailable()).thenReturn(false);
    ...
    void addComment_shouldReturnCommentDto_whenValid() {
        ...
        when(userRepository.findById(authorId)).thenReturn(Optional.of(author));
        when(itemRepository.findById(itemId)).thenReturn(Optional.of(item));
        when(bookingRepository.findFirstByItemIdAndBookerIdAndStatusAndEndBefore(anyLong(), anyLong(), any(), any())).thenReturn(Optional.of(booking));
    ...
    void getAllCommentsForOwner_shouldReturnList_whenCommentsExist() {
        long ownerId = 1L;
        Comment comment = new Comment();
        when(commentRepository.findAllByItemOwnerId(ownerId)).thenReturn(List.of(comment));
        List<CommentDto> result = itemService.getAllCommentsForOwner(ownerId);
        assertNotNull(result);
        assertEquals(1, result.size());
    }
    ...
    void createItem_shouldReturnItemDto_whenRequestIdPresent() {
        ...
        owner.setId(ownerId);
        PostItemRequest request = mock(PostItemRequest.class);
        when(request.getRequestId()).thenReturn(10L);
        Item item = new Item();
    ...
    void patchItem_shouldNotChangeFields_whenAllFieldsNull() {
        ...
        owner.setId(ownerId);
        item.setOwner(owner);
        PatchItemRequest request = new PatchItemRequest(); // все поля null
        when(itemRepository.findById(itemId)).thenReturn(Optional.of(item));
        when(itemRepository.save(any())).thenReturn(item);
        ItemDto result = itemService.patchItem(itemId, request, ownerId);
        assertNotNull(result);
    ...
    void fillBookingDates_shouldReturn_whenBookingsEmpty() {
        ...
        assertNotNull(result);
        assertEquals(1, result.size());
        // lastStart/lastEnd/nextStart/nextEnd должны быть null
    ...
    void fillBookingDates_shouldSetLastBooking_whenOnlyPastBookings() {
        ...
        past.setItem(item);
        when(bookingRepository.findAllByItemIdIn(anyList())).thenReturn(List.of(past));
        List<OwnedItemDto> result = itemService.getOwnerItems(ownerId);
        assertNotNull(result);
        assertEquals(1, result.size());
    ...
    void fillBookingDates_shouldSetNextBooking_whenOnlyFutureBookings() {
        ...
        future.setItem(item);
        when(bookingRepository.findAllByItemIdIn(anyList())).thenReturn(List.of(future));
        List<OwnedItemDto> result = itemService.getOwnerItems(ownerId);
        assertNotNull(result);
        assertEquals(1, result.size());
        assertNull(result.get(0).getLastStart());
    ...
    void addComment_shouldThrow_whenNoApprovedBooking() {
        ...
        when(userRepository.findById(authorId)).thenReturn(Optional.of(author));
        when(itemRepository.findById(itemId)).thenReturn(Optional.of(item));
        // Нет завершённого подтверждённого бронирования
        when(bookingRepository.findFirstByItemIdAndBookerIdAndStatusAndEndBefore(anyLong(), anyLong(), any(), any())).thenReturn(Optional.empty());
        assertThrows(ArgumentsNotValidException.class, () -> itemService.addComment(request, itemId, authorId));
    }
}
